...
class EntryAd(BasePage):
    ...
    def go_to_page(self):
        link = self.page.locator(EntryAdLocators.LOADED_PAGE)
        link.click()

    def click_re_enable_button(self):
        button = self.page.locator(EntryAdLocators.RE_ENABLE_BUTTON)
        button.click()

    def click_close_popup(self):
        self.should_be_popup_present()
        button = self.page.locator(EntryAdLocators.CLOSE_BUTTON)
        button.click()
